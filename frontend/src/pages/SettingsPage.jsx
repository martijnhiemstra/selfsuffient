...
export const SettingsPage = () => {
  ...
  const fetchOpenaiSettings = async () => {
    try {
      ...
      if (res.data.model) setOpenaiModel(res.data.model);
    } catch (err) {
  ...
  const handleChangePassword = async (e) => {
    ...
    setChanging(true);
    try {
      await axios.post(`${API}/auth/change-password`, { current_password: currentPassword, new_password: newPassword }, { headers });
  ...
  const handleToggleDailyReminders = async (checked) => {
    ...
    try {
      await axios.put(`${API}/auth/settings`, { daily_reminders: checked }, { headers });
  ...
  const handleSaveOpenaiSettings = async () => {
    if (!openaiApiKey) { toast.error('Please enter your OpenAI API key'); return; }
    setSavingOpenai(true);
    try {
      await axios.post(`${API}/openai/settings`, { api_key: openaiApiKey, model: openaiModel }, { headers });
  ...
  const handleTestOpenaiKey = async () => {
    if (!openaiApiKey) { toast.error('Please enter your OpenAI API key to test'); return; }
    setTestingOpenai(true);
    try {
      const res = await axios.post(`${API}/openai/test`, { api_key: openaiApiKey, model: openaiModel }, { headers });
      res.data.valid ? toast.success(res.data.message) : toast.error(res.data.message);
    } catch (err) {
  ...
  const handleDeleteOpenaiSettings = async () => {
    ...
    } finally { setDeletingOpenai(false); }
  };

  const handleSaveGoogleSettings = async () => {
    if (!googleClientId || !googleClientSecret) { toast.error('Please enter both Client ID and Client Secret'); return; }
    setSavingGoogleSettings(true);
    try {
      await axios.post(`${API}/google-calendar/settings`, { client_id: googleClientId, client_secret: googleClientSecret, sync_tasks: syncTasks, sync_routines: syncRoutines, sync_events: syncEvents }, { headers });
      toast.success('Google Calendar settings saved');
      setGoogleClientId(''); setGoogleClientSecret('');
      fetchGoogleCalendarStatus();
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Failed to save settings');
    } finally { setSavingGoogleSettings(false); }
  };

  const handleUpdateSyncSettings = async () => {
    if (!googleCalendarStatus?.has_credentials) { toast.error('Please save your credentials first'); return; }
    setSavingGoogleSettings(true);
    try {
      await axios.post(`${API}/google-calendar/settings`, { client_id: googleCalendarStatus.client_id || 'unchanged', client_secret: googleCalendarStatus.client_secret || 'unchanged', sync_tasks: syncTasks, sync_routines: syncRoutines, sync_events: syncEvents }, { headers });
  ...
  const handleSyncRoutines = async () => {
    ...
    } finally { setSyncingRoutines(false); }
  };
  ...
  return (
    <div className="p-6 md:p-12 lg:p-16 max-w-3xl" data-testid="settings-page">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4 mb-6" data-testid="settings-tabs">
            ...
            <CardContent>
              <form onSubmit={handleChangePassword} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="currentPassword" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Enter current password" required data-testid="current-password-input" />
                </div>
                <Separator />
                <div className="space-y-2">
                  <Label htmlFor="newPassword">New Password</Label>
                  <Input id="newPassword" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Enter new password" required minLength={6} data-testid="new-password-input" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="confirmPassword">Confirm New Password</Label>
                  <Input id="confirmPassword" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm new password" required minLength={6} data-testid="confirm-password-input" />
                </div>
                <Button type="submit" disabled={changing} data-testid="change-password-button">
                  {changing ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <CheckCircle className="w-4 h-4 mr-2" />}
                  Change Password
                </Button>
              </form>
            </CardContent>
          ...
          <Card className="border border-border/50">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Switch id="daily-reminders" checked={dailyReminders} onCheckedChange={handleToggleDailyReminders} disabled={savingReminders} data-testid="daily-reminders-switch" />
              </div>
              {dailyReminders && (
        ...
        <TabsContent value="calendar" className="space-y-6">
          <Card className="border border-border/50">
            <CardHeader>
              ...
              <CardDescription>Sync your tasks and routines to Google Calendar</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* Connection Status */}
              {googleCalendarStatus?.connected ? (
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Button onClick={handleConnectGoogle} disabled={connectingGoogle}>
                      {connectingGoogle ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Link className="w-4 h-4 mr-2" />}
                      Connect
                    </Button>
                  </div>
                </div>
                ...
                <div className="bg-muted/50 rounded-lg p-4">
                  <p className="text-sm text-muted-foreground">Enter your Google Cloud OAuth credentials below to enable calendar sync.</p>
                </div>
              )}
                ...
                <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="googleClientId" value={googleClientId} onChange={(e) => setGoogleClientId(e.target.value)} placeholder="xxxxx.apps.googleusercontent.com" data-testid="google-client-id" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="googleClientSecret">Client Secret</Label>
                      <Input id="googleClientSecret" type="password" value={googleClientSecret} onChange={(e) => setGoogleClientSecret(e.target.value)} placeholder="GOCSPX-xxxxx" data-testid="google-client-secret" />
                    </div>
                    <Button onClick={handleSaveGoogleSettings} disabled={savingGoogleSettings || !googleClientId || !googleClientSecret}>
                      {savingGoogleSettings ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <CheckCircle className="w-4 h-4 mr-2" />}
                      Save Credentials
                    </Button>
                  </div>
                </>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Switch id="sync-routines" checked={syncRoutines} onCheckedChange={setSyncRoutines} />
                      </div>
                      <div className="flex items-center justify-between">
                        <div>
                        ...
                        <Switch id="sync-events" checked={syncEvents} onCheckedChange={setSyncEvents} disabled />
                      </div>
                    </div>
                  </div>
                  <Separator />
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" onClick={handleSyncTasks} disabled={syncingTasks || !syncTasks}>
                        {syncingTasks ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <RefreshCw className="w-4 h-4 mr-2" />}
                        Sync All Tasks
                      </Button>
                      <Button variant="outline" onClick={handleSyncRoutines} disabled={syncingRoutines || !syncRoutines}>
                        {syncingRoutines ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <RefreshCw className="w-4 h-4 mr-2" />}
        ...
        <TabsContent value="ai" className="space-y-6">
          <Card className="border border-border/50">
            <CardHeader>
              ...
              <CardDescription>
                Configure your OpenAI API key for AI-powered features like transaction analysis and garden design
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* Status */}
              {openaiSettings?.has_api_key ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-green-600 dark:text-green-400">
                          Key: {openaiSettings.api_key_preview} &bull; Model: {openaiSettings.model}
                        </p>
                      </div>
                    </div>
                    <Button variant="outline" size="sm" onClick={handleDeleteOpenaiSettings} disabled={deletingOpenai}>
                      {deletingOpenai ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Trash2 className="w-4 h-4 mr-2" />}
                      Remove
                    </Button>
                  </div>
                </div>
                ...
                <div className="bg-muted/50 rounded-lg p-4">
                  <p className="text-sm text-muted-foreground">
                    Add your OpenAI API key to enable AI-powered features. Your API key is stored encrypted and used only for your account.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline flex items-center gap-1">
                    Get API Key <ExternalLink className="w-3 h-3" />
                  </a>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="openaiApiKey">API Key</Label>
                  <Input id="openaiApiKey" type="password" value={openaiApiKey} onChange={(e) => setOpenaiApiKey(e.target.value)} placeholder="sk-..." data-testid="openai-api-key" />
                </div>
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">Choose based on your preference for speed vs accuracy. GPT-4o Mini is recommended for most users.</p>
                </div>
                <div className="flex gap-2">
                  <Button onClick={handleTestOpenaiKey} variant="outline" disabled={testingOpenai || !openaiApiKey}>
                    {testingOpenai ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <TestTube className="w-4 h-4 mr-2" />}
                    Test Key
                  </Button>
                  <Button onClick={handleSaveOpenaiSettings} disabled={savingOpenai || !openaiApiKey}>
                    {savingOpenai ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Key className="w-4 h-4 mr-2" />}
                    Save API Key
                  </Button>
                </div>
              </div>
              ...
              <div className="text-sm text-muted-foreground space-y-2">
                <p className="font-medium text-foreground">AI is used for:</p>
                <ul className="list-disc list-inside space-y-1">
                  <li>Transaction analysis &mdash; auto-categorize imported transactions and detect recurring payments</li>
                  <li>Garden Designer &mdash; generate AI-powered garden layouts with plant placement suggestions</li>
